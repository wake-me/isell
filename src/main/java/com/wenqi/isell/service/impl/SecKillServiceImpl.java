...
import com.wenqi.isell.service.RedisLock;
import com.wenqi.isell.service.SecKillService;
...
public class SecKillServiceImpl implements SecKillService {
    ...
    public void orderProductMockDiffUser(String productId)
    {
        ...
        if (!redisLock.lock(productId,String.valueOf(time))){
            throw new ISellException(100,"哎呦喂，人也太多了，换个姿势");
        }
        //1.查询该商品库存，为0则活动结束。
        int stockNum = stock.get(productId);
        if(stockNum == 0) {
            throw new ISellException(100,"活动结束");
        }else {
            //2.下单(模拟不同用户openid不同)
